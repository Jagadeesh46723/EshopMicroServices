...
namespace Catalog.API.Products.GetProductById
{
    ...
    public class GetProductByIdHandler(IDocumentSession session) : IQueryHandler<GetProductByIdQuery, GetProductByResult>
    {
        public async Task<GetProductByResult> Handle(GetProductByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            return response is null ? throw new ProductNotFoundException(request.id) : new GetProductByResult(response);
        }
    }
}
